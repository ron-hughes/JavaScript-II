...
function runEach() { runners.forEach(function(item) {

    fullName.push(item.first_name);
});
}
...
let mapped = runners.map((e) => {
    return allCaps.push(e.first_name.toUpperCase());
...
let taxDonate = runners.reduce((total, donate) => {
    
    return total + donate.donation;
  
}, 0);

ticketPriceTotal.push(taxDonate);
...
// Figure out which person gave the biggest donation and print that users name along with donation amount.

// const bigArr = [];
// const bigDonation = runners.forEach(function(item, index, array){
//          if (Math.max(runners.donation) ) {
//             bigArr.push(runners.donation)
//             return console.log(bigArr)
//          }
// });
